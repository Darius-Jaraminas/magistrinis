...
z.int <- function(u = y, interval = c(0, 0.9999)){
  t <- length(u)
  sj <- t(u[-t]) %*% z.matrix(u = u[-1])
...
sim.null <- function(N, t, d){
  ...
  for (i in 1:N){
    ...
    s1 <- c(s1, null.tests(y = y1))
    s2 <- c(s2, null.tests(y = y2))
    s3 <- c(s3, null.tests(y = y3))
    s4 <- c(s4, null.tests(y = y4))
  }
  ...
  size[1, ] <- t.size(m = s1, d = d)
  size[2, ] <- t.size(m = s2, d = d)
  size[3, ] <- t.size(m = s3, d = d)
  size[4, ] <- t.size(m = s4, d = d)
...
null.tests <- function(y){
  ...
  avr <- Auto.VR(y)
  zint <- z.int(u = y)
  rz1 <- rz(u = y, m = 1, alpha = 0.05)
  rz1m <- rz(u = y, m = 5, alpha = 0.05)
  rz2 <- rzsq(u = y, m = 1, alpha = 0.05)
  r <- c(aq$Stat, avr, zint$stat.std, rz1$stat, rz1m$stat, rz2$stat)
  names(r) <- c("aq", "avr", "zint", "rz1", "rz1m", "rz2")
  return(r)
}
...
z.sq <- function(theta, u){
  zt <- z(theta = theta, u = u)^2
  return(zt)
}
...
rz <- function(u, m = 1, alpha = 0.05, theta = NULL){
  if (m == 1){
    q1 <- qnorm(alpha/(2*m))
    q2 <- qnorm(1-alpha/(2*m))
    if (is.null(theta)){
      theta <- runif(1)
    }
    zs <- as.numeric(z(theta = theta, u = u))
    t <- (zs > q1) & (zs <= q2)
    re <- list(result = t, stat = zs, theta = theta)
  }
  else{
...
rz.boot <- function(u, m = 1, alpha = 0.05, B){
  stat <- rz(u = u, m = m, alpha = alpha)
  boot.stat <- numeric(B)
  for (i in 1:B){
    e <- rnorm(length(u))
    u.star <- u * e
    boot.stat[i] <- rz(u = u.star, m = m, alpha = alpha, theta = stat$theta)$stat
  }
  q1 <- quantile(boot.stat, alpha/2)
  q2 <- quantile(boot.stat, 1 - alpha/2)
  ### calculation of pvalue !!!!!!!!!!!!! must be checked
  m <- mean(boot.stat)
  if (stat$stat < m){
    pval <- 2 * mean(stat$stat > boot.stat)
  } else{
    pval <- 2 * mean(stat$stat < boot.stat)
  }
  t <- (stat$stat > q1) & (stat$stat < q2)
  names(t) <- NULL
  r <- list(stat = stat$stat, pval = pval, t = t)
  return(r)
}

rzsq <- function(u, m = 1, alpha = 0.05, theta = NULL){
  q <- qchisq(1-alpha/m, df = 1)
  if (is.null(theta)){
    theta <- runif(1)
  }
  zs <- as.numeric(z.sq(theta = theta, u = u))
  t <- zs <= q
  t <- all(t)
  r <- list(result = t, stat = zs, theta = theta)
  return(r)
}
...
z.cov <- function(x){
  l <- length(x)
  co <- matrix(1, nrow = l, ncol = l)
  for (i in 1:l){
    for (j in 1:l){
      co[i, j] <- sqrt((1-x[i]^2)*(1-x[j]^2))/(1-x[i]*x[j])
    }
  }
  return(co)
}

z.cov.sim <- function(theta, N, t, dgp = "normal"){
  zt <- matrix(NA, nrow = N, ncol = length(theta))
  for (j in 1:N){
    u <- gen.null(n = t, method = dgp)
    for (i in 1:length(theta)){
      zt[j, i] <- z(theta = theta[i], u = u)
    }
  }
  co <- cor(zt)
  return(co)
...
sim.alternative <- function(N, t, B, h1){
